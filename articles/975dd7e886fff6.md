...
直接`mise.toml`を作成して記述することでプロジェクトの環境を定義させることも可能です。
...
設定ファイルがあれば環境構築時にカレントディレクトリで`mise install`するだけで構築完了してしまいます。
またディレクトリに入ると自動的に指定されたバージョンが有効にもなってくれます。

:::message
自動でバージョン有効にするには現在のシェルで`mise activate`が必要になります。
:::
...
# config_rootでmiseが解決するプロジェクトルートのパスが取得できます
...
開発ツールのバージョン管理に加えて、OSパッケージやdotfilesを含むPC全体のセットアップをmiseで管理したい場合は、`mise bootstrap`を紹介した次の記事も参照してください。

https://zenn.dev/takuty/articles/770b88416b5e6c
